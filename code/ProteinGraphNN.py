...
def pattern_set_to_input_target_dicts(pattern_set):
    ...
    sum_region = 0
    inst_index = len(pattern_set.region_node_list)
    for num_region, num_inst in zip(pattern_set.num_regions_per_pattern, pattern_set.num_instances_per_pattern):
        for i in range(num_inst):

            pattern_g_senders.extend(range(sum_region, sum_region+num_region))
            pattern_g_receivers.extend([inst_index]*num_region)
            pattern_edges.extend([1/num_inst]*num_region)
            pattern_edge_targets.extend(
                        [1 if pattern_set.instance_membership_targets[n] == i else 0
                            for n in range(sum_region, sum_region+num_region)])

            pattern_g_senders.extend([inst_index]*num_region)
            pattern_g_receivers.extend(range(sum_region, sum_region+num_region))
            pattern_edges.extend([1/num_inst]*num_region)
            pattern_edge_targets.extend(
                        [1 if pattern_set.instance_membership_targets[n] == i else 0
                            for n in range(sum_region, sum_region+num_region)])
    ...
    pattern_edges = np.reshape(np.array(pattern_edges, dtype=np.float32), (-1,1))
    ...
    return sequence_dict, pattern_dict, target_dict
...
class ProteinGNN(snt.AbstractModule):
    ...
    def _build(self,
                ...
                pattern_graph, #graphs tuple with a single graph for the pattern graph
                num_processing_steps):

        latent_seq_graph = self.sequence_encoder(sequence_graph)
        latent_seq_graph0 = latent_seq_graph

        latent_pattern_graph = self.pattern_encoder(pattern_graph) #only encodes edges
